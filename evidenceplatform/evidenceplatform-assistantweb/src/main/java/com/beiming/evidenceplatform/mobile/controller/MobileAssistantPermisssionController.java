...
@RestController
@RequestMapping("auth/mobile/assistant/permission")
@Slf4j
public class MobileAssistantPermisssionController {
  ...
  public Result assistLogin(@RequestBody AssistUserLoginDTO assistUserLoginDTO) {
    ...
    try {
      log.info("辅助人员用户名(手机号)密码登录。\n手机号：{} \n密码：{}", assistUserLoginDTO.getPhone(),
          md5Password);
      AssistantTokenUserDTO assistantTokenUserDTO = assitantUserManage
          .assitantUserLogin(assistUserLoginDTO.getPhone(), md5Password);
      ...
      //生成token传入参数对象
      LoginInfoResponseDTO loginInfoResponseDTO = tokenGenerator
          .convertUserDetail2TokenInfo(assistantTokenUserDTO.getUserId(),
              assistantTokenUserDTO.getActualName(), assistantTokenUserDTO.getSocialIdentify(),
              assistantTokenUserDTO.getTypeList());
      ...
      LoginTokenResponseDTO loginTokenResponseDTO = tokenGenerator
          .generateLoginToken(loginInfoResponseDTO);
  ...
  public Result systemLogin(@RequestBody SystemUserLoginDTO systemUserLoginDTO) {
    ...
    try {
      log.info("系统管理员用户名(手机号)密码登录。\n手机号：{} \n密码：{}", systemUserLoginDTO.getPhone(),
          md5Password);
      SystemTokenUserDTO systemTokenUserDTO = assitantUserManage
          .systemUserLogin(systemUserLoginDTO.getPhone(), md5Password);
      if (null == systemTokenUserDTO) {
        return Result.failed(ErrorCode.USER_NOT_EXISTS, "用户密码错误或用户不存在");
      }
      //生成token传入参数对象
      LoginInfoResponseDTO loginInfoResponseDTO = tokenGenerator
          .convertUserDetail2TokenInfo(systemTokenUserDTO.getUserId(),
              systemTokenUserDTO.getActualName(), systemTokenUserDTO.getSocialIdentify(),
              systemTokenUserDTO.getTypeList());
  ...
  public Result courtManageLogin(@RequestBody CourtManageUserDTO courtManageUserDTO) {
    ...
    try {
      log.info("系统管理员户名(手机号)密码登录。\n手机号：{} \n密码：{}", courtManageUserDTO.getPhone(),
          md5Password);
      CourtManageTokenUserDTO courtManageTokenUserDTO = assitantUserManage
          .courtManageUserLogin(courtManageUserDTO.getPhone(), md5Password);
      ...
      //生成token传入参数对象
      LoginInfoResponseDTO loginInfoResponseDTO = tokenGenerator
          .convertUserDetail2TokenInfo(courtManageTokenUserDTO.getUserId(),
              courtManageTokenUserDTO.getActualName(),
              courtManageTokenUserDTO.getSocialIdentify(),
              courtManageTokenUserDTO.getTypeList());
  ...
  @PostMapping("/assisterUserChangePwd")
  public Result assisterUserChangePwd(
    ...
    Assistanter assistanter = assitantUserManage
        .selectAssistanterById(ContextUtil.getCurrentUserId());
    if (null == assistanter) {
    ...
    if (assitantUserManage
        .assistanterChangePwdByUserId(ContextUtil.getCurrentUserId(),
  ...
  @PostMapping("/assistanterUser/ForgetPwd")
  public Result assistanterUserForgetPwd(
      @RequestBody AssistanterForgetPwdDTO assistanterForgetPwdDTO) {

    String smsCode = redisService
        .get(RedisKey.SMS_CODE_JUDSALE, assistanterForgetPwdDTO.getMobilePhone());
    log.info("查看竞拍者手机号是否存在。\n用户手机号：{}", assistanterForgetPwdDTO.getMobilePhone());
    if (assitantUserManage
        .assistantUserIsExit(null, assistanterForgetPwdDTO.getMobilePhone(), null)) {
    ...
    if (assitantUserManage
        .assistanterChangePwdByUserId(null, assistanterForgetPwdDTO.getMobilePhone(),
            MD5Util.string2MD5(assistanterForgetPwdDTO.getPassword()))) {
      return Result.success();
    }
    return Result.failed(ErrorCode.CHANGE_PASSWORD_FAIL, "修改密码失败");
