...
//      String filePath = (javax.servlet.http.HttpServletRequest)request.getSession().getServletContext().getRealPath("/") + "resources/images/logoImages/llhlogo.png";
    ...
    String filePath = "/Users/xiet/Pictures/59364982.jpg";  //TODO
   ...
   * 给二维码图片添加Logo
   */
  public String addLogoQRCode(BufferedImage bim, File logoPic, LogoConfig logoConfig,
      String productName) {
    try {
      /**
       * 读取二维码图片，并构建绘图对象
      ...
      int widthLogo = logo.getWidth(null) > image.getWidth() * 3 / 10 ? (image.getWidth() * 3 / 10)
          : logo.getWidth(null);
      int heightLogo =
          logo.getHeight(null) > image.getHeight() * 3 / 10 ? (image.getHeight() * 3 / 10)
              : logo.getWidth(null);
...
//                  outg.drawString(productName, 0, image.getHeight() + (outImage.getHeight() - image.getHeight())/2 + 5 ); //画文字
          ...
          String productName2 = productName
              .substring(productName.length() / 2, productName.length());
      ...
      ImageIO.write(image, "png",
          new File("/Users/xiet/Pictures/123.jpg")); //TODO
   ...
   * @param hints 设置参数
   */
  public BufferedImage getQRCodeBufferedImage(String content, BarcodeFormat barcodeFormat,
